...
export async function createLoanApplication(applicationData) {
  try {
    const { user_id, ...appData } = applicationData;
    
    // Store the application data as JSON string
    const dataString = JSON.stringify(appData);
    
    const result = await client.execute({
      sql: `INSERT INTO loan_applications (user_id, application_data, status)
            VALUES (?, ?, 'pending');`,
      args: [user_id, dataString]
    });
...
export async function getLoanApplicationsByUserId(userId) {
  try {
    const result = await client.execute({
      sql: `SELECT id, user_id, application_data, status, created_at, updated_at
    ...
    return result.rows.map(row => {
      ...
      if (obj && obj.application_data) {
        try {
          obj.application_data = JSON.parse(obj.application_data);
        } catch (e) {
