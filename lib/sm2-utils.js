...
export function encryptSM2(data, publicKey = DEFAULT_PUBLIC_KEY) {
  try {
    ...
    console.log('[SM2 Debug] Encrypting data, length:', data.length);
    console.log('[SM2 Debug] Processed key length:', processedKey.length);
    console.log('[SM2 Debug] Processed key starts with:', processedKey.substring(0, 4));
    
    // sm-crypto mode 0 = C1C2C3 (matching Java SM2Engine.Mode.C1C2C3)
    // mode 1 = C1C3C2
    // Java uses C1C2C3, so we use mode 0
    // 
    // Note: sm-crypto.doEncrypt(data, publicKey, mode)
    // According to documentation, publicKey should include '04' prefix (130 chars)
    ...
    console.log('[SM2 Debug] Encryption successful, result length:', encrypted.length);
    return encrypted;
  } catch (error) {
    console.error('[SM2 Error] Encryption failed:', error);
    console.error('[SM2 Error] Error stack:', error.stack);
    console.error('[SM2 Error] Public key length:', publicKey ? publicKey.length : 'null');
    console.error('[SM2 Error] Public key preview:', publicKey ? publicKey.substring(0, 20) + '...' : 'null');
    console.error('[SM2 Error] Data type:', typeof data);
    console.error('[SM2 Error] Data length:', data ? data.length : 'null');
