...
export default withCors(async function handler(req, res) {
  ...
  // Handle down/file path (action will be "down/file" in Vercel)
  if (action && action.includes('down')) {
    if (req.method !== 'GET') {
      return res.status(405).json({ code: 405, message: 'Method not allowed. Use GET.' });
    }
    return handleDownFile(req, res);
  }
  ...
  switch (action) {
    ...
    default:
      return res.status(404).json({
        code: 404,
        message: `Unknown action: ${action}. Valid actions: getBannerList, syncApplication, upload/file, down/file`
      });
  }
});
...
/**
 * Handle down file (get banner image)
 * GET /api/integration/down/file?id=<banner_id>
 * 
 * Query parameters:
 ...
 */
async function handleDownFile(req, res) {
  try {
    const { id } = req.query;
